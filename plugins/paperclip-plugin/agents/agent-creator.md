...
Pre-generate has already created the directory structure, `runtime/config.toml`, and `AGENTS.md` YAML frontmatter. You only write the creative content.
...
**Rules:**
- List every plugin from the agent's `runtime/config.toml`
