...
Quick reference for assigning marketplace plugins and MCP servers to agents based on their role.
...
When a plugin ships MCP servers, enabling the plugin makes those tools available through Codex.
...
2. **Env vars:** set `AGENT_EMAIL`, `COMPANY_DOMAIN`, and `GOOGLE_WORKSPACE_CLI_CREDENTIALS_FILE` in `runtime/config.toml` under `[shell_environment_policy.set]` (all three as a bundle)
