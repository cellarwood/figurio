...
│           ├── config.toml             # Plugins, env vars, MCP servers
...
│   ├── config.toml                     # Global Codex bootstrap settings (baseline)
...
|---|---|---|
| `global/config.toml` | `/paperclip/.codex/config.toml` or bootstrap defaults | All agents |
| `global/plugins.json` | `/docker-init/codex/plugins.json` | All agents |
| `agents/{slug}/runtime/config.toml` | `<workspace>/.codex/config.toml` | One agent |
