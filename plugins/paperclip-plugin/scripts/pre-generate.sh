...
#   - global/config.toml (shared Codex plugin bootstrap)
#   - global/plugins.json (union of all agent plugins)
#   - Per-agent runtime/config.toml (plugins, env, MCP servers)
...
{
  "marketplaces": [
    {"name": "codex-my-marketplace", "source": "lukaskellerstein/codex-my-marketplace", "scope": "user"}
  ],
...
# =============================================================================
# Step 5: Generate per-agent runtime/config.toml
# =============================================================================
...
for i in $(seq 0 $((AGENT_COUNT - 1))); do
  ...
  config_file="$COMPANY_ROOT/agents/$slug/runtime/config.toml"
  cat > "$config_file" << 'RUNTIME_CONFIG'
[plugins."paperclip-plugin@codex-my-marketplace"]
enabled = true
RUNTIME_CONFIG

  for plugin in $agent_plugins; do
    cat >> "$config_file" << PLUGIN_TOML

[plugins."${plugin}-plugin@codex-my-marketplace"]
enabled = true
PLUGIN_TOML
  done
  ...
  if is_gws_eligible "$role" && [ -n "$GWS_DOMAIN" ]; then
    ...
    escaped_agent_email=$(escape_toml_string "$agent_email")
    escaped_gws_domain=$(escape_toml_string "$GWS_DOMAIN")
    escaped_gws_creds=$(escape_toml_string "$GWS_CREDENTIALS_FILE")
    cat >> "$config_file" << GWS_TOML

[shell_environment_policy]
inherit = "all"

[shell_environment_policy.set]
AGENT_EMAIL = "$escaped_agent_email"
COMPANY_DOMAIN = "$escaped_gws_domain"
GOOGLE_WORKSPACE_CLI_CREDENTIALS_FILE = "$escaped_gws_creds"
GWS_TOML
  fi

  if [ "$chrome_mcp" = "true" ]; then
    cat >> "$config_file" << 'CHROME_TOML'
...
CHROME_TOML
  ...
  echo "  $slug: paperclip-plugin + $(echo "$agent_plugins" | wc -w | tr -d ' ') role plugins$(is_gws_eligible "$role" && [ -n "$GWS_DOMAIN" ] && echo " + GWS" || true)$( [ "$chrome_mcp" = "true" ] && echo " + Chrome MCP" || true)"
done
...
echo "  - agents/*/runtime/config.toml"
