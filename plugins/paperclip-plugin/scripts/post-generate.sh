...
for agent_dir in "$COMPANY_ROOT/agents"/*/; do
  ...
  if [ -f "$agent_dir/runtime/config.toml" ]; then
    ok "agent-files" "$slug/runtime/config.toml exists"
  else
    error "agent-files" "$slug/runtime/config.toml is missing"
  fi
done
...
for slug in "${AGENT_SLUGS[@]}"; do
  config_toml="$COMPANY_ROOT/agents/$slug/runtime/config.toml"
  [ ! -f "$config_toml" ] && continue

  if ! config_json=$(load_toml_json "$config_toml" 2>/dev/null); then
    error "plugin-assignment" "$slug: runtime/config.toml is not valid TOML"
    continue
  fi

  enabled=$(printf '%s' "$config_json" | jq -r '.plugins // {} | keys[]' 2>/dev/null)
  for plugin_key in $enabled; do
    if ! echo "$plugin_key" | grep -qE '^(paperclip-plugin|[a-z-]+-plugin)@codex-my-marketplace$'; then
    ...
    if [ "$plugin_key" != "paperclip-plugin@codex-my-marketplace" ] && [ -n "$GLOBAL_PLUGINS" ]; then
  ...
  if ! echo "$enabled" | grep -q '^paperclip-plugin@codex-my-marketplace$'; then
    error "plugin-assignment" "$slug: runtime/config.toml must enable paperclip-plugin@codex-my-marketplace"
  fi

  # Check plugin-specific MCP sections exist when needed
  mcp_servers=$(printf '%s' "$config_json" | jq -r '.mcp_servers // {} | keys[]' 2>/dev/null | tr '\n' ' ')
  for plugin_key in $enabled; do
    plugin_name=$(echo "$plugin_key" | sed 's/@.*//')
    if [ "$plugin_name" = "web-design-plugin" ] && ! echo " $enabled " | grep -q ' design-plugin@codex-my-marketplace'; then
      error "plugin-deps" "$slug: $plugin_name requires design-plugin but it's not enabled"
    fi
  ...
  if echo " $mcp_servers " | grep -q ' chrome-devtools '; then
    ok "chrome-mcp" "$slug: runtime/config.toml defines chrome-devtools MCP"
  fi
done
...
for slug in "${AGENT_SLUGS[@]}"; do
  ...
  if [ -f "$config_toml" ] && config_json=$(load_toml_json "$config_toml" 2>/dev/null); then
    [ "$(printf '%s' "$config_json" | jq -r '.shell_environment_policy.set.AGENT_EMAIL // empty' 2>/dev/null)" != "" ] && has_agent_email=true
    [ "$(printf '%s' "$config_json" | jq -r '.shell_environment_policy.set.COMPANY_DOMAIN // empty' 2>/dev/null)" != "" ] && has_company_domain=true
    [ "$(printf '%s' "$config_json" | jq -r '.shell_environment_policy.set.GOOGLE_WORKSPACE_CLI_CREDENTIALS_FILE // empty' 2>/dev/null)" != "" ] && has_gws_creds=true
  fi
...
for slug in "${AGENT_SLUGS[@]}"; do
  config_toml="$COMPANY_ROOT/agents/$slug/runtime/config.toml"
  if [ -f "$config_toml" ] && config_json=$(load_toml_json "$config_toml" 2>/dev/null); then
    if printf '%s' "$config_json" | jq -e '.mcp_servers["chrome-devtools"]' &>/dev/null; then
      ok "chrome-mcp" "$slug: runtime/config.toml defines Chrome DevTools MCP"
    fi
  fi
done
...
for slug in "${AGENT_SLUGS[@]}"; do
  ...
  for sa_file in "$sa_dir"/*.md; do
    ...
    if [ -n "$sa_fm_desc" ]; then
      ...
      error "subagents" "$slug/$sa_name: missing 'description' in frontmatter (Codex uses this to decide when to delegate)"
    fi
